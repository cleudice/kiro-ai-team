...
| `steering/escalation-rules\|quality-gates\|workflow` | installer | install/update | **só via update do ai-team** (regra do time; mudança local = PR no central) | todos, sempre |
| `steering/guidelines/*` | installer | install/update | via update do ai-team | agente do stack, por `fileMatch` |
...
regras do time (escalation/gates/workflow/guidelines)  ← ai-team, via update
   ▼
...
3. Humano: **confere `tech.md`** (os comandos de build/teste são o que `implement-task`, `verify-change` e `merge-gate` executam — comando errado aqui quebra os gates) e **escreve `product.md`** (5–10 linhas; o *porquê* não está no código).
...
## 5. O que é nativo do Kiro vs. do ai-team

| Nativo (docs oficiais) | ai-team (este repo) |
|---|---|
