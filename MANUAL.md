...
| `.kiro/specs/<slug>/tasks.md` | `write-tasks` | por PBI (2 trilhos) | **execução é o "Start task" nativo do Kiro** (marca `[x]`); `task-preflight` cerca com pré-flight/checkpoint; escopo não muda sem voltar à spec | dev (via nativo), orchestrator |
...
3. Humano: **confere `tech.md`** (os comandos de build/teste são o que `task-preflight`, o "Start task" nativo, `verify-change` e `merge-gate` executam — comando errado aqui quebra os gates) e **escreve `product.md`** (5–10 linhas; o *porquê* não está no código).
