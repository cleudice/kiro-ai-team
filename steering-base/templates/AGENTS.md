...
3. **Nenhum merge sem os gates G1–G5** — verificados por `check-gates.sh`. Caminho: `.kiro/skills/merge-gate/scripts/check-gates.sh` no escopo padrão (project); se `.kiro/.kiro-ai-team-paths` existir (escopo hybrid/global), resolva primeiro com `ENGINE="$(bash .kiro/scripts/kiro-paths.sh)"` e rode `"$ENGINE/skills/merge-gate/scripts/check-gates.sh"`.
