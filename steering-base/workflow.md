...
Execução: `task-preflight` (pré-flight/checkpoint) + "Start task" nativo do Kiro (worktree próprio) em paralelo com `write-blackbox-tests` (worktree QA separado, sem `src/`) → `verify-change` → `review-spec` + `review-code` → `merge-gate` → `resolve-issue` no tracker de origem.
