...
- **Evidência amarrada ao commit (B2)**: G2/G3/G4 e o log de G5 precisam da linha `Commit: <sha>`, e esse sha precisa ser (prefixo d)o HEAD atual de `pbi/<ID>` — aprovar num commit e commitar código novo depois no mesmo PBI invalida a aprovação; `check-gates.sh` reprova mecanicamente até o gate ser refeito sobre o commit atual.
